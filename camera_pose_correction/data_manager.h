...
public:
	...
	{                                
		...
		/*projectionMat<<  -0.9966,    0.0214,   -0.0797,  0.5487686,// updated on 18/04/2016
						0.0233,    0.9995,   -0.0232,  -0.0858538,
						0.0792,   -0.0249,   -0.9965,  0.2873345;*/

		projectionMat<< -0.9993 ,  -0.0060 ,  -0.0382 , 0.5496248,
						-0.0064  ,  0.9999  ,  0.0110 , -0.0674437,
						 0.0381 ,   0.0113  , -0.9992 , 0.3282176;
		
		/*CI<< 2176.211068, 0.000000, 1244.289249, 0.000000, 2178.633804, 1041.836938, 0.000000, 0.000000, 1.000000;*/  
		CI<< 2328.556745, 0.000000 ,1254.058997, 0.000000 ,2334.311366, 1012.072590 ,0.000000, 0.000000, 0.999071;//updated on 18/04/2016
	...
	for (int profile_no=0;profile_no<ctr;profile_no++)
		{
			...
			float x = (*arr)[profile_no][i][2],y = (*arr)[profile_no][i][3],z = (*arr)[profile_no][i][4],a = (*arr)[profile_no][i][5]* PI / 180.0,b = (*arr)[profile_no][i][6]* PI / 180.0,c = (*arr)[profile_no][i][7]* PI / 180.0;
				...
				/*T_T_L<<-0.7986,   -0.6007,   -0.0370,  113.9513,
						-0.6004,    0.7995,   -0.0201,   45.4287,
						 ...
						 0,         0,         0,    1.0000;*/
				/*T_T_L<<        -0.7872,   -0.6154,   -0.0402,  115.7029, // 18/4/2016 updated by shraddha with new calibration data
								-0.6158,    0.7879,   -0.0024,   41.0518,
								0.0332,    0.0228,   -0.9992,  328.6233,
									0 ,        0   ,      0  ,  1.0000;*/
					T_T_L<< 	   -0.8364,   -0.5475,   -0.0263 , 113.6588,
									-0.5478 ,   0.8366 ,   0.0045,   35.7817,
									0.0195 ,   0.0182 ,  -0.9996 , 331.6524,
										 0 ,        0 ,        0 ,   1.0000;
				//std::cout << T_T_L<<endl;
			...
			for(i=0; i<1280; i++)
			{
				...
				basic_point.x = baseCoord[0]-1.5;
				basic_point.y = baseCoord[1] - 0.5;
				basic_point.z = baseCoord[2];
				cloud->points.push_back(basic_point);
			}
...
void projectCloud( pcl::PointCloud<pcl::PointXYZ>::Ptr cloud , cv::Mat boat2D)
{
	...
	pcl::PCDWriter writer;
	writer.write ("my_point_cloud_projectedCloud.pcd", *cloud);
